...
function merge(
	...
	const userGroup = typeof options[key] === 'object' ? (options[key] as Record<string, any>) : {}
	// eslint-disable-next-line ts/no-unsafe-assignment, ts/no-explicit-any
	const legacyOverrides = (options.overrides as Record<string, any> | undefined)?.[key]
...
export async function factory(
	...
	const userFusedConfig: TypedFlatConfigItem = {}
	const cleanOptions: OptionsConfig = { ...options }
	...
	if (cleanOptions.typescript !== false) {
		const userTypescript: OptionsTypescript & OptionsTypeScriptWithTypes = typeof cleanOptions.typescript === 'object' ? cleanOptions.typescript : {}
	...
	return await antfu(
		cleanOptions,
		Object.keys(userFusedConfig).length > 0 ? userFusedConfig : {},
		...userConfigs,
	)
}
